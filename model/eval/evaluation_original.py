...
def get_utility_metrics(real_path,fake_paths,scaler="MinMax",type={"Classification":["lr","dt","rf","mlp"]},test_ratio=.20):
    ...
    # print("fake data result:", all_fake_results_avg)
    print("fake data f1-score: ", np.array(all_fake_results_avg).mean(axis=0)[:,2], np.array(all_fake_results_avg).mean(axis=0)[:,2].mean())
...
def stat_sim(real_path,fake_path,cat_cols=None):
    ...
    for column in real.columns:
        ...
        if column in cat_cols:
            ...
            l1 = real[column].values
            l2 = fake[column].values
